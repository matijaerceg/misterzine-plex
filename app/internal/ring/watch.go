...
// itself in one diagnostics report. It stops when stop is closed.
func (r *Ring) Watch(logf func(string, ...any), stop <-chan struct{}) {
	...
	for n := 0; ; n++ {
		...
		if time.Since(report) >= 5*time.Second {
			if lostHeader+foreign+black > 0 {
				logf("watch: in 5 s the header was lost %d times, published by another writer %d times, and the published slot read black %d times",
